...
public class TalkNPC10_mission : MonoBehaviour {
    ...
    public GameObject selectBiodiversityobject;    //設定一個物件,可以通用
    public string onselectBiodiversityobjname;

    public string setfungusselectBiodiversityobjname // 設定值給fungus(中毒香蕉)
    {
        get { return Mission_Flowchart.GetStringVariable("selectBiodiversityobjname"); }
        set { Mission_Flowchart.SetStringVariable("selectBiodiversityobjname", value); }
    }
    public string Mission //抓fuguns裡面的值
    {
        get { return Mission_Flowchart.GetStringVariable("Mission_Biodiversity"); } //指定到對應的欄位
        set { Mission_Flowchart.SetStringVariable("Mission_Biodiversity", value); }
    }
    ...
    public void Awake()
    {
        IconExclamationIcons = GameObject.Find("ExclamationIcons10"); //初始化抓該NPC的驚嘆號	
        IconExclamationIconsForMap = GameObject.Find("NPC-10Quest");
    }
    // Use this for initialization
    void Start()
    {
        selectBiodiversityobject = GameObject.Find(onselectBiodiversityobjname);
        seletobjecttip = GameObject.Find(objecttipname);
        seletobjecttip.SetActive(false);
        if (Loadcharacter.MissionComplete[13] == true) //如果從資料庫拉資料發現已經玩過這關
        {
    ...
    void OnTriggerStay(Collider other) //觸發開始
    {
        ...
        else
        {
            ...
            if (other.gameObject.CompareTag("Player") && Input.GetKeyDown(KeyCode.Z))
            {
                ...
                setfungusselectBiodiversityobjname = onselectBiodiversityobjname;
    ...
    void Mission_Biodiversity() //任務接受後,執行函式
    {
        if (Mission == "Ing")
        {
            IconExclamationIcons.SetActive(false);
            MissionUI.NotCompletedMission[12].SetActive(false);
            MissionUI.ProcessingMisssion[12].SetActive(true); //UI開啟
            MissionUI.CompletedMission[12].SetActive(false);
            MissionUI.MissionView.SetActive(true);
            //InformationViewUIscript.InformationView.SetActive(true);
            MissionListUI.MissionListUIstate[12].SetActive(true);
        }
        if (Mission == "Done")
        {
            MissionUI.NotCompletedMission[12].SetActive(false);
            MissionUI.ProcessingMisssion[12].SetActive(false); //UI開啟
            MissionUI.CompletedMission[12].SetActive(true);
    ...
    {
        Debug.Log("關閉選擇物件");
        selectBiodiversityobject.SetActive(false); 
